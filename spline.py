...
class Spline(object):
    ...
    def __init__(self, t, c, k):
        ...
        if (self.c.ndim < 1):
            raise ValueError
            ("Coefficients must be at least 1-dimensional.")
        if (self.c.shape[0] < n):
            raise ValueError(
    ...
    def eval_coef(self, x, m=0):
        """Evaluate the coefficients of B-Splines at given value
        for up to the m-th derivative.

        Parameters
        ----------
        x : float
            The value at which the coefficients of BSplines
            are to be evaluated
        m : int, optional
            Indicates the m-th derivative
    ...
    def de_Boor(self, x, m=0):
        """Evaluate the spline function at given value
        for up to the m-th derivative.
        ...
        # Search for knot span and return index
        index = self._search_index(x)
        # Initialize points to the coefficients of Bsplines according
        if (m == 0):
        ...
        for i in range(p - s):
            ...
            for j in range(p - s - i):
                ...
                right = j + index - p + i + 1
                alpha = (x - self.t[right]) / \
                    (self.t[left] - self.t[right])
                points[j] = (1 - alpha) * points[j] + alpha * points[j+1]
        return points[0]

    def interpolate(self, d, u):
        """Evaluate the control points fo an interpolating spline curve
        given a set of data points and parameters.
        ...
        if (len(param) != dpoints.shape[0]):
            raise ValueError(
                "Need exactly as many data points as parameters.")
    ...
    def _colloc(self, u):
        ...
        according to the given parameter vector.
    ...
    def _search_index(self, x):
        """Find the index for a value within a knot span
        [t_index,t_index+1) of the knot vector.
...
def haus_dist(points, spl, x):
    """Calculate the Hausdorff distance between a given B-Spline curve
    and a data polygon defined by a set of given points.

    Parameters
    ----------
    points : array_like (2, n)
        A list of sample vector arrays
        representing the data polygon in 2D
    spl :
        B-Spline object
    x : array_like
    ...
    for i in range(n):
        ...
        min21 = float("inf")
        for j in range(n):
            # Min Distance from the ith point
            # on the curve (1) to the data polygon (2)
            dist12 = math.sqrt(
                (math.pow((points1[i][0]-points2[j][0]), 2) +
                 math.pow((points1[i][1]-points2[j][1]), 2)))
            min12 = dist12 if dist12 < min12 else min12
            # Min Distance from the ith point
            # on the data polygon (2) to the curve (1)
            dist21 = math.sqrt(
                (math.pow((points1[j][0]-points2[i][0]), 2) +
                 math.pow((points1[j][1]-points2[i][1]), 2)))
...
def eval_angles(partitions, spl):
    """Calculate the approximated angles of the partitions of a given
    B-Spline curve.
    The partitions are defined by their chord lengths.

    Parameters
    ----------
    partitions : array_like (n, 2)
        A list of arrays with entries for angles and chord lengths
        representing the desired partitions of the curve
    spl :
        B-Spline object
    ...
    def Len(x):
        return math.sqrt(math.pow(spl(x, m=1).T[0], 2) +
                         math.pow(spl(x, m=1).T[1], 2))
    ...
    for i in range(len(parti)):
        ...
        if (parti[i][0] == 0):
            ...
            u_start = scipy.optimize.brentq(
                lambda x: scipy.integrate.quad(Len, 0, x, limit=100)[0]
                - start, 0, 1)
            ...
            # Intersection angle of the normal lines
            results[i] = np.degrees(np.arctan(
                np.absolute(((dX1/dY1)-(dX2/dY2)) /
                            (1+(dX1/dY1)*(dX2/dY2)))))
        # Update chord
        chords += parti[i][1]
    return results
